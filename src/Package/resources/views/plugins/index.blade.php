                            ...
                            <div class="col" id="{{ $cardId }}">
                                <x-core::card class="h-100 entomai-plugin-card">
                                    ...
                                    <div
                                        ...
                                        style="height: 110px; background: linear-gradient(135deg, rgba(var(--bb-primary-rgb),.07) 0%, rgba(var(--bb-primary-rgb),.03) 100%); border-bottom: 1px solid var(--bb-border-color);"
                                    >
            ...
            try {
                ...
                grid.innerHTML = items.map(function (item) {
                    const iconHtml = item.icon_url
                        ? '<img src="' + escHtml(item.icon_url) + '" alt="" style="width:48px;height:48px;object-fit:contain;border-radius:8px;">'
                        : '<span class="avatar avatar-lg rounded bg-primary-lt"><i class="ti ti-puzzle text-primary" style="font-size:1.8rem;"></i></span>';
                    ...
                    if (item.is_marketplace_free && item.marketplace_search_query && marketplaceBaseUrl) {
                        ...
                        actionHtml = '<a href="' + escHtml(mktUrl) + '" class="btn btn-success btn-sm w-100"><i class="ti ti-download me-1"></i>Download from Marketplace</a>';
                    } else if (item.is_free && item.download_url) {
                        actionHtml = '<a href="' + escHtml(item.download_url) + '" target="_blank" class="btn btn-success btn-sm w-100"><i class="ti ti-download me-1"></i>Download</a>';
                    } else if (! item.is_free && item.buy_url) {
                        actionHtml = '<a href="' + escHtml(item.buy_url) + '" target="_blank" class="btn btn-primary btn-sm w-100"><i class="ti ti-shopping-cart me-1"></i>Buy</a>';
                    }

                    const tagsHtml = Array.isArray(item.tags) && item.tags.length
                        ? '<div class="mt-2 d-flex flex-wrap gap-1">' + item.tags.map(t => '<span class="badge bg-secondary-lt text-secondary">' + escHtml(t) + '</span>').join('') + '</div>'
                        : '';
